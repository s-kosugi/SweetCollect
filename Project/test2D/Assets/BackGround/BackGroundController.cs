    ...
    void Update()
    {
        if (m_GamemainManager.state == GameMainManager.STATE.MAIN)
        {
            ...
            if (transform.position.x < -100 && !MainCameraIn)
            {
                ...
                if (TraceObject)
                {
                    transform.position = new Vector3(TraceObject.transform.position.x + GetComponent<SpriteRenderer>().bounds.size.x - 1.5f, transform.position.y, transform.position.z);
                }
            }
        }
